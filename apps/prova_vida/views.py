...
from apps.corecode.models import Categoria, DirecaoAlocacao, CategoriaNova, DireccaoDepartamento, Estacao, FuncaoChefia
...
def provaGetFuncDetail(request) :
    ...
    estacao  = Estacao.objects.filter(estado_objecto='activo')
    observacao  = ProvaVida.objects.filter()
    direccaoDepart = DireccaoDepartamento.objects.filter(estado_objecto='activo')
...
def efectuarProvaVida(request):
    if request.method == 'POST':
        ...
        try:
         direccaoDepart = DireccaoDepartamento.objects.get(pk=request.POST['departament_direcao'])
        except:
         direccaoDepart=None   

        try:
         estacao= Estacao.objects.get(pk=request.POST['estacao'])
        except:
         estacao=None
        try:
        ...
        employee.estacao = estacao
        employee.direccaoDepart= direccaoDepart
...
#Profissionais
            ...
            str(obj.funcionario.funcao_chefia) if obj.funcionario.funcao_chefia is not None else '',
            str(obj.funcionario.funcao_chefia_nova) if obj.funcionario.funcao_chefia_nova is not None else '',            
            obj.funcionario.vencimento_mensal,       
            obj.funcionario.habilitacao,
            obj.funcionario.area_de_formacao,
            obj.observacao,
