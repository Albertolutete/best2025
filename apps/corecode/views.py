...
@login_required(login_url="/accounts/login/")
def direccaodepartamento_add(request):

    abertura = Abertura_Prova_Vida.objects.filter(estado_actual="aberta").first()

    direccaodp = DireccaoDepartamento(nome=request.POST["nome"])
    direccaodp.save()
    context = {"direccaodp":direccaodp,"abertura_actual":abertura}
...
@login_required(login_url="/accounts/login/")
def estacao_add(request):

    abertura = Abertura_Prova_Vida.objects.filter(estado_actual="aberta").first()

    estacao = Estacao(nome=request.POST["nome"])
    estacao.save()
    context = {"estacao":estacao,"abertura_actual":abertura}
...
def estacao_list(request):
    ...
    template_name = "corecode/estacao_list.html"
    return render(request, template_name,context)


@login_required(login_url="/accounts/login/")
def departamentodirec_list(request):
    abertura = Abertura_Prova_Vida.objects.filter(estado_actual="aberta").first()
    ...
    template_name = "corecode/departamentoDireccao_list.html"
    return render(request, template_name,context)
